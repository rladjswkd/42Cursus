...
namespace ft{
	...
	rb_tree_node_base *ft::find_leftmost(const rb_tree_node_base *node) {
		while (node->left)
			node = node->left;
		return (const_cast<rb_tree_node_base *>(node));
	}

	rb_tree_node_base *ft::find_rightmost(const rb_tree_node_base *node) {
		while (node->right)
			node = node->right;
		return (const_cast<rb_tree_node_base *>(node));
	}

	rb_tree_node_base *ft::do_increase(const rb_tree_node_base *node) {
		if(node->right)  {
			node = node->right;
			while (node->left)
				node = node->left;
...
//	in case 2., if there are more than one nodes, if statement is true. else, if statement is false. so in both cases, &sentinel is returned.
	...
	rb_tree_node_base *ft::do_decrease(const rb_tree_node_base *node) {
